//C++ Reference Code for Java files and php/html conversion
...
double GetTestAverage(int Number_of_Tests)
{
	...
	q1 = "Enter Test " + to_string(counter) + ":             ";
	double t = 0;
	double ts;
	double cavg = 0;
	while (counter <= Number_of_Tests)
	{
		q2 = "Enter Test " + to_string(counter) + ":                 ";
...
int main()
{
	...
	PrintLogo();
	cout << "To stop enter q for first name!" << endl;
	cout << "------------------------------------------------------------\n";
	...
	while (f_name != "q")
	{
		...
		cout << "**********************************\n";
		PrintStudent(f_name, tavg, pavg, davg, fscore, l_grade);
		counter++;
		favg += fscore;
